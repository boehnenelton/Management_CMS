...
RELATIONAL_ID:  108b11b0-0d17-45d5-bd12-77f10ee97255
"""
...
def bejson_core_sort_by_field(doc: dict, field_name: str, reverse: bool = False) -> None:
    idx = bejson_core_get_field_index(doc, field_name)
    if idx == -1: return
    # BUG FIX: previously used `x[idx] if x[idx] is not None else ""` as the
    # sort key, which raises TypeError for any non-string field (e.g.
    # "integer") once a row with a real None value and a row with a real
    # int value both appear — Python 3 doesn't allow comparing str and int.
    # Fixed with a (is_none, value) tuple key: None-valued rows sort
    # together (last, by default) without ever comparing None against a
    # differently-typed real value.
    doc["Values"].sort(key=lambda x: (x[idx] is None, x[idx]), reverse=reverse)
